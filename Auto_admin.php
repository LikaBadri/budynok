...
if(!empty($_COOKIE["AdminCookie"])){
    $data = json_decode($_COOKIE["AdminCookie"], true);
        ...
        $query = $db->prepare("SELECT pass_admin FROM admin_admin WHERE id_admin = :id");
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = $db->prepare("SELECT pass_admin, id_admin FROM stud_admin WHERE email_admin = :email");
    $query->execute([':email'=>$email]);
    $result = $query->fetch();

    if($pass == $result["pass_admin"]){
        $data = json_encode(["id_admin"=>$result["id_admin"], "pass_admin"=>$result["pass_admin"]]);
        setcookie("AdminCookie", $data, time()+86400, "/");  /* срок действия 1 час */
    } else {
        header("Location: Auto_admin.php");
    }
}
