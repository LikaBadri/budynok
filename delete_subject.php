...
if(!empty($_POST)){
    ...
    try {
        ...
        $query = $db->prepare("DELETE FROM stud_subject WHERE name_subject = :name_subject");
        

        $query->execute(array('name_subject' => $name_subject));
...
<header>
    ...
    <section>
    <h3>Видаленя даних про предмет в розкладі</h3>
    <form action="delete_subject.php" method="post">
        <div>
            <label for="name_subject">Виберіть предмет для видалення:</label><br>
            <select id="name_subject" name="name_subject" required>
                <!-- Выводим список предметов из базы данных -->
                <?php 
                try {
                    $sql = "SELECT name_subject FROM stud_subject";
                    $query = $db->query($sql);
        ...
        <button type="submit" style="margin-left: 5% ;width: 15%; height: 50%;">Видалити дані предмету</button>
    </form>
</section>
