<?php
include __DIR__."/chekcookie.php"; // Подключаем файл с проверкой авторизации

if(isset($_POST['submit'])) {
    // Проверяем, является ли пользователь администратором
    if($IsAdmin) {
        // Проверяем, какую таблицу нужно обработать на удаление
        if(isset($_POST['stud_email'])) {
            // Удаление данных из таблицы stud
            $stud_email = $_POST['stud_email'];
            $sql = "DELETE FROM stud WHERE stud_email = :stud_email";
            $stmt = $pdo->prepare($sql);
            $stmt->bindParam(':stud_email', $stud_email, PDO::PARAM_STR);
            $stmt->execute();
            $message = "Дані учня видалені.";
        } else if(isset($_POST['guide_email'])) {
            // Удаление данных из таблицы stud_guide
            $guide_email = $_POST['guide_email'];
            $sql = "DELETE FROM stud_guide WHERE guide_email = :guide_email";
            $stmt = $pdo->prepare($sql);
            $stmt->bindParam(':guide_email', $guide_email, PDO::PARAM_STR);
            $stmt->execute();
            $message = "Дані працівника видалено.";
        }
    } else {
        // Если пользователь не администратор, выводим сообщение об ошибке
        $message = "В вас немає права на видалення даних.";
    }
}
?>
<!DOCTYPE html>
...
<header>
    ...
    <section>
    <h3>Виделеня даних</h3>
    <!-- Выводим сообщение об успехе или ошибке, если оно есть -->
    <?php if(isset($message)) { echo "<p>$message</p>"; } ?>
    
    <form action="" method="post">
        <div>
            <label for="stud_email">Введіть пошту учня для видалення:</label><br>
            <input type="email" id="stud_email" name="stud_email" required><br>
        </div>
    ...
    <form action="" method="post">
        <div>
            <label for="guide_email">Введіть пошту працівників для видалення:</label><br>
            <input type="email" id="guide_email" name="guide_email" required><br>
        </div>
        <button type="submit" name="submit">Удалить преподавателя</button>
    </form>
</section>
