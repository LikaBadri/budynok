...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $query = $db->prepare("SELECT pass_stud, id_stud FROM stud WHERE email_stud = :email");
    $query->execute([':email'=>$email]);
    $result = $query->fetch();
    
    if($pass == $result["pass_stud"]){
        $data = json_encode(["id_stud"=>$result["id_stud"], "pass_stud"=>$result["pass_stud"]]);
        setcookie("StudCookie", $data, time()+86400, "/");  /* срок действия 1 час */
    } else {
        header("Location: Auto.php");
    }
}
