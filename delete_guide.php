...
if(!empty($_POST)){
    ...
    try {
        ...
        $query = $db->prepare("DELETE FROM stud_guide WHERE email_guide = :email_guide");
        

        $query->execute(array('email_guide' => $email_guide));
...
<header>
    ...
    <section>
        <h3>Видалення даних працівників</h3>
        <!-- Выводим сообщение об успехе или ошибке, если оно есть -->
        <?php if(isset($message)) { echo "<p>$message</p>"; } ?>
        
        <form action="delete_guide.php" method="post">
            <div>
                <label for="email_guide">Введіть пошту працівника для видалення:</label><br>
                <input type="email_guide" id="email_guide" name="email_guide" required><br>
            </div>
            <button type="submit" name="submit" style="margin-left: 5% ;width: 15%; height: 50%;">Видалити дані працівника</button>
        </form>
    </section>
