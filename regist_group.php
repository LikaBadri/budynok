<?php
// Подключаем файл проверки авторизации
...
if(!empty($_POST)){
    // Ваши переменные и подключение к базе данных...
    $name_group = $_POST['name_group'];

    // Подключение к базе данных и проверка на ошибки...

    $query = $db->prepare("INSERT INTO stud_group (name_group) VALUES (:name_group)");

    $query->execute([":name_group" => $name_group]);

    // После выполнения операции перенаправляем пользователя
    header("Location: process_form.php");
    exit; // Добавляем exit после перенаправления, чтобы прекратить выполнение кода на этой странице
}
?>
...
<header>
    ...
    <section>
        <h3>Реєстрація групи</h3>
        <form action="Registr_group.php" method="post">
            <div>
                <label for="name_group">Назва групи:</label><br>
    ...
    <button type="submit" style="margin-left: 5% ;width: 15%; height: 50%;">Зареєструвати</button>
</form>
</section>
