...
try{
   ...
   $query = $db->prepare("INSERT INTO stud_subject (name_subject, day_subject, id_guide, id_group) 
    VALUES (:name_subject, :day_subject, :id_guide, :id_group)");

    $query->execute([":name_subject" => $name_subject,":day" => $day,":id_guide" => $ide_guide, ":id_group" => $id_group]);



     header("Location: process_form_admin.php");
}
?>
...
<header>
    ...
    <section>
    <h3>Реєстрація розкладу</h3>
    <form action="regist_guide.php" method="post">
        <div>
            <label for="name_subject">Назва предмету:</label><br>
            <input type="text" id="name_subject" name="name_subject" required><br>
        </div>
        <div>
            <label for="day">День тижня:</label><br>
            <input type="text" id="day" name="day" required><br>
  
            <label for="guide">Вчитель:</label>
            <select id="guide" name="guide">
                <?php
                // Подключение к базе данных
                $connection = mysqli_connect('127.0.0.1', 'root', '', 'budynok');
                ...
                // SQL-запрос для получения списка учителей
                $sql = "SELECT first_name_guide, last_name_guide, middle_name_guide FROM stud_guide";
                $result = mysqli_query($connection, $sql);
    
                // Вывод списка учителей в выпадающем списке
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        $full_name = $row['first_name_guide'].' '. $row['last_name_guide'].' '.$row['middle_name_guide'];
                        echo "<option value='{$full_name}'>{$full_name}</option>";
                    }
                } else {
                    echo "<option disabled>Нема таких даних</option>";
                }
        ...
        <div>
            <label for="group_name">Виберіть групу:</label>
            <select id="group_name" name="group_name">
                ...
                // SQL-запрос для получения списка групп
                $sql = "SELECT name_group FROM stud_group";
                $result = mysqli_query($connection, $sql);
    
                // Вывод списка групп в выпадающем списке
                if (mysqli_num_rows($result) > 0) {
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo "<option value='{$row['name_group']}'>{$row['name_group']}</option>";
                    }
                } else {
                    echo "<option disabled>Нема груп</option>";
                }
        ...
        <button type="submit" style="margin-left: 5% ;width: 10%; height: 50%;">Зареєструватися</button>
</form>
</section>
